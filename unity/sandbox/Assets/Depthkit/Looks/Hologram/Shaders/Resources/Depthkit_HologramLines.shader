...
Shader "Depthkit/HologramLines"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float _Opacity;
            float _BlendEnum;
            uint _LineOrientation; // 0 = vertical, 1 = horizontal
            ...
            void geom(line v2g input[2], inout TriangleStream<v2g> OutputStream)
            {
                ...
                float depth0 = mul(_InverseExtrinsics, float4(input[0].worldPos, 1)).z;
                float depth1 = mul(_InverseExtrinsics, float4(input[1].worldPos, 1)).z;

                if (depth0 <= (_NearClip + DEPTH_EPS) ||
                    depth0 >= (_FarClip  - DEPTH_EPS) ||
                    depth1 <= (_NearClip + DEPTH_EPS) ||
                    depth1 >= (_FarClip  - DEPTH_EPS))
                {
                    return;
                }
                ...
                float3 up = _LineOrientation == 0 ? float3(1, 0, 0) : float3(0, 1, 0);
                ...
                for(int i = 0; i < 2; i++)
                {
                    ...
                    output.worldPos = vert0.xyz;
                    output.vertex = mul(UNITY_MATRIX_VP, vert0);
                    output.uv = float2(i == 0 ? 0.0 : 1.0, 0.0);
                    ...
                    output.worldPos = vert1.xyz;
                    output.vertex = mul(UNITY_MATRIX_VP, vert1);
                    output.uv = float2(i == 0 ? 0.0 : 1.0, 1.0);
                    output.uv_MainTex   = input[i].uv_MainTex;
                    output.uv2_MainTex2 = input[i].uv2_MainTex2;
