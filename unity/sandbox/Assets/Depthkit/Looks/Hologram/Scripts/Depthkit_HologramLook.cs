...
namespace Depthkit
{
    ...
    public class Depthkit_HologramLook : Depthkit_ClipRenderer
    {
        public struct ShaderBlendMode
        {
            public Shader s;
            public Depthkit_HologramLook.BlendMode m;
            public ShaderBlendMode(Shader s, Depthkit_HologramLook.BlendMode m)
            {
                this.s = s;
                this.m = m;
            }
        ...
        private Depthkit_HologramLookOccludeLayer _occludeLayer;
        private Depthkit_HologramLookFillLayer _fillLayer;
        private Depthkit_HologramLookLayer[] _layers;

        /// <summary>
        /// Self occlusion property so the hologram doesn't blend with itself
        /// <summary>
        public bool _selfOcclusion = true;

        public override RenderType GetRenderType()
        {
#if DK_USING_HOLOGRAM
        ...
        public override void SetGeometryDirty()
        {
            ...
            if (_layers != null)
            {
                foreach (Depthkit_HologramLookLayer layer in _layers)
                {
                    if (layer != null)
                    {
        ...
        public override void Draw()
        {
            ...
            {
                _layers = GetComponentsInChildren<Depthkit_HologramLookLayer>();
                if (_layers == null)
                {
        ...
        {
            _occludeLayer = GetComponent<Depthkit_HologramLookOccludeLayer>();
            bool addedLayer = false;
            if(_occludeLayer == null)
            {
                _occludeLayer = gameObject.AddComponent<Depthkit_HologramLookOccludeLayer>();
                _occludeLayer.hideFlags = HideFlags.HideInInspector;
                addedLayer = true;
            }

            _fillLayer = GetComponent<Depthkit_HologramLookFillLayer>();
            if (_fillLayer == null)
            {
                _fillLayer = gameObject.AddComponent<Depthkit_HologramLookFillLayer>();
            ...
            if(!_initialized)
            {
                ...
                Depthkit_HologramLookLineLayer lineLayer = gameObject.AddComponent<Depthkit_HologramLookLineLayer>();
                Depthkit_HologramLookPointLayer pointLayer = gameObject.AddComponent<Depthkit_HologramLookPointLayer>();
        ...
        {
            gameObject.AddComponent<Depthkit_HologramLookLineLayer>();
        }

        public void AddPointLayer()
        {
            gameObject.AddComponent<Depthkit_HologramLookPointLayer>();
        }
